...
What is held equal between a backbone's FPN arm and its BiFPN arm:
  ...
  * every level refined by a 3x3 convolution in every pass
  * the same CBAM block on each level entering the head

so the difference left is the fusion topology: a top-down pyramid against
bidirectional flow with learnable weights and an input-to-output skip. The
parameter counts still differ slightly, because the topologies need different
numbers of convolutions per pass; that is part of what is being compared. The
...
def fpn_pass(L, p3, p4, p5, w, tag):
    """
    One top-down pass, with every level refined by a 3x3 convolution.

    P5 used to carry through as its bare 1x1 lateral: it reached the head
    without the 3x3 smoothing every other level had, and a second pass
    upsampled that same unrefined lateral again, so P5 never changed between
    passes. BiFPN refines P5 in every pass, so the two necks differed by more
    than topology. Now every level is refined and each pass starts from the
    previous pass's output, at every level, in both necks.
    """
    p5_out = L.add(p5, "Conv", f"[{w}, 3, 1]", f"P5 out {tag}")
    L.add("[" + f"{up(L, p5_out)}, {p4}" + "]", "Concat", "[1]")
...
def build(backbone, neck, width, repeats):
    ...
    if not is_yolo:
        out += [
            "# Width, pass count and per-level refinement are shared with the matching",
            f"# {other} configuration, so the two arms differ in fusion topology.",
            "#",
        ]
    out += [
